...
#logcases
#new entry for each case
df<-data.frame()
#colnames(df)<-c("lon", "lat","year","week","state","city","address")
df<- subcases[rep(row.names(subcases), subcases$cases), 1:7]
hist(as.numeric(df$week),breaks=52)
